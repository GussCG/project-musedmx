...
# Acomodar por orden alfabetico por nombre de museo y quitar espacios en blanco
df['museo_nombre'] = df['museo_nombre'].str.strip()
df = df.sort_values(by='museo_nombre')

# Eliminar Duplicados
df = df[df['museo_nombre'] != 'Museo de Arte de la Secretaria de Hacienda y Crédito Público (SHCP)']
df = df[df['museo_nombre'] != 'Museo La Ciudad y la Torre a través de los siglos']
df = df[df['museo_nombre'] != 'Museo del Ejército y Fuerza Aérea Mexicanos de Bethlemitas (Cerrado Temporalmente)']
