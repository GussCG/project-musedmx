...
export default function MuseoCard({museo}) {
    ...
    const [isFavorite, setIsFavorite] = useState(user?.favoritos.includes(museo.id) || false)
    ...
    const agruparHorarios = () => {
        ...
        Object.keys(horariosAgrupados).forEach((horario) => {
            horariosAgrupados[horario].sort((a, b) => diasSemana.indexOf(a) - diasSemana.indexOf(b))
        })
    
        // Formatear el resultado
        let resultado = Object.entries(horariosAgrupados).map(([horario, dias]) => {
            ...
            if (dias.length === 1) {
                ...
                for (let i = 1; i < dias.length; i++) {
                    ...
                    if (diasSemana.indexOf(diaActual) === diasSemana.indexOf(diaAnterior) + 1) {
                ...
                return agrupados.map((grupo) => {
                    return grupo.length === 1 ? `${grupo[0]}` : `${grupo[0]} - ${grupo[grupo.length - 1]}`
                }).join(', ') + `: ${horario}`
            }
        }).join('\n')
        ...
        <div className="museo-card">
            ...
            <div className="museo-card-info-container">
                <div className="museo-card-info-header">
                    <Link to={`/Museos/${museoInfo.id}`} id="museo-card-nombre">{museoInfo.nombre}</Link> 
                </div>
